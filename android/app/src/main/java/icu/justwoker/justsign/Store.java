...
public class Store {
    ...
    private void migrateLocked(JSONObject cfg) {
        try {
            ...
            /* auth-v2 日志格式与旧授权链路不兼容。升级后一次性清空两条旧日志，
             * 避免历史 OAuth/会话指纹记录被误判为本版行为；账号和业务数据不受影响。 */
            if (!sp.getBoolean("migrated_auth_v2_logs", false)) {
                sp.edit().putString("opLogs", "[]").putString("logs", "[]")
                        .putBoolean("migrated_auth_v2_logs", true).commit();
            }
    ...
    public void moveSiteTo(String siteKey, int targetIndex) {
        if (siteKey == null) return;
        synchronized (LOCK) {
            try {
                JSONObject cfg = config();
                JSONArray arr = cfg.optJSONArray("sites");
                if (arr == null || targetIndex < 0 || targetIndex >= arr.length()) return;
                JSONObject moving = null;
                JSONArray rest = new JSONArray();
                for (int i = 0; i < arr.length(); i++) {
                    JSONObject s = arr.optJSONObject(i);
                    if (s != null && siteKey.equals(s.optString("key"))) moving = s;
                    else if (s != null) rest.put(s);
                }
                if (moving == null) return;
                JSONArray out = new JSONArray();
                int insert = Math.min(targetIndex, rest.length());
                for (int i = 0; i <= rest.length(); i++) {
                    if (i == insert) out.put(moving);
                    if (i < rest.length()) out.put(rest.optJSONObject(i));
                }
                cfg.put("sites", out);
                saveConfigLocked(cfg);
            } catch (Exception ignored) {}
    ...
    public void moveSite(String siteKey, int delta) {
        if (siteKey == null || delta == 0) return;
        synchronized (LOCK) {
            try {
                JSONObject cfg = config();
                JSONArray arr = cfg.optJSONArray("sites");
                ...
                for (int i = 0; i < arr.length(); i++) {
                    JSONObject s = arr.optJSONObject(i);
                    if (s != null && siteKey.equals(s.optString("key"))) { from = i; break; }
                }
                int to = from + delta;
                if (from < 0 || to < 0 || to >= arr.length()) return;
                JSONArray out = new JSONArray();
                for (int i = 0; i < arr.length(); i++) {
                    if (i == from) out.put(arr.optJSONObject(to));
                    else if (i == to) out.put(arr.optJSONObject(from));
                    else out.put(arr.optJSONObject(i));
                }
                cfg.put("sites", out);
                saveConfigLocked(cfg);
            } catch (Exception ignored) {}
