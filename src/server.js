...
async function handleApi(req, res, url, body) {
  ...
  if (cmd === 'status' && seg[2]) {
    ...
    const result = {
      ...
      affQuotaUSD: d.aff_quota != null ? +(d.aff_quota / unit).toFixed(2) : null,
      siteUserId: String(d.id || x.acc.siteUserId || '')
    };
    /* v0.2.0：划转开关开启时，刷新顺带执行邀请额度划转（对齐安卓 v0.6.0） */
    if (cfg.affTransfer?.enabled && d.aff_quota != null && Number(d.aff_quota) >= unit) {
      try {
        const tr = await cl.call('post', '/api/user/aff_transfer', { quota: Number(d.aff_quota) });
        appendLog({ site: x.site.key, account: x.acc.key, event: 'aff-transfer',
          detail: { http: tr.status, code: tr.data?.code, ok: tr.status === 200 && tr.data?.success !== false } });
      } catch (_) {}
    }
